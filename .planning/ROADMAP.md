...
- [x] 01-05-PLAN.md — Landing + /sources + /privacy (PIPA) + /terms + admin stub + disclaimer + cookie notice (wave 5)
- [x] 01-06-PLAN.md — DSAR endpoint + email verification + Resend + CPO forwarding alias (wave 5)
- [x] 01-07-PLAN.md — Sentry (client/server/edge) + PII scrubbing + Vercel Analytics + heartbeat + storage monitor (wave 5)
- [x] 01-08-PLAN.md — [BLOCKING] supabase db push + Vercel deploy + end-to-end smoke tests (wave 6)
...
- [ ] 03.1-02-PLAN.md — Primitives: npx shadcn add chart slider avatar + Badge filter-chip variant + metrics/{Sparkline, Delta, LogoTile} + neutral-scrub audit + 5 unit tests (wave 2)
- [ ] 03.1-03-PLAN.md — Shell: header brand wordmark with lime dot + Geist font + footer/disclaimer token migration off text-neutral-* (wave 2, parallel with 03.1-02)
- [ ] 03.1-04-PLAN.md — /search rework: ActiveFilterChips (Badge filter-chip) + FacetRangeInputs (Slider overlay) + FacetGroup (Geist Mono meta-label headers) + ResultsHeader (live-count pulse) + SearchInput (top-bar shadow) + ko.json liveLabel key (wave 3)
- [ ] 03.1-05-PLAN.md — / landing: minimal brand hero (wordmark + headline + tagline + 검색 시작 CTA → /search + LIVE status pill) + landing.hero3_1 i18n namespace + smoke test (wave 3, parallel with 03.1-04)
